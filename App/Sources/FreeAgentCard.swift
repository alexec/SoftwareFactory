...
/// Starting an agent on no project: a browser owner, a reviewer, anything that works
/// across the factory. You say what it is for; it starts in your home folder.
///
/// It lived on the Agents page, which came off the sidebar in T360, and it was the only
/// way to start an agent that is not on a project. Losing that was not what was asked
/// for, so it is its own card now and sits on the No project page, where such an agent
/// belongs. (T360, Alex, 15 Sep 2026.)
